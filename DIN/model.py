...
from DIN.modules import Attention_Layer, Dice
...
class DIN(Model):
    ...
    def call(self, inputs):
        ...
        # 输入部分确定，这里看起来是0代表缺失，但正常id从0开始，可以考虑-1
        # mask = tf.cast(tf.not_equal(seq_inputs[:, :, 0], 0), dtype=tf.float32)  # (None, maxlen)
        mask = tf.cast(tf.not_equal(seq_inputs[:, :, 0], -1), dtype=tf.float32)  # (None, maxlen)
        # other
        ...
        # att
        # query: item_embed, batch, d*2
        # key, value: seq_embed, batch, maxlen, d*2
